...
import { useSalesInvoices } from '../../hooks/salesInvoiceQueries';
...
export function SalesInvoiceList() {
  ...
  const {
    ...
    fetchNextPage,
    isFetchingNextPage
  } = useSalesInvoices(debouncedSearch, statusFilter);
  ...
  const handleAdd = useCallback(() => {
    navigation.navigate('NewSalesInvoice');
  }, [navigation]);

  const getStatusStyles = useCallback((status: string) => {
    if (!status) return { text: colors.primary, bg: colors.blue_50 };
    switch (status.toLowerCase()) {
      case 'paid': return { text: colors.success, bg: colors.green_100 };
      case 'unpaid': return { text: colors.error, bg: colors.red_100 };
      case 'overdue': return { text: colors.warning, bg: colors.orange_100 };
      case 'draft': return { text: colors.neutral_600, bg: colors.neutral_100 };
      case 'cancelled': return { text: colors.neutral_400, bg: colors.neutral_100 };
  ...
  const renderItem = useCallback(({ item }: any) => {
    ...
    return (
      <TouchableOpacity 
        onPress={() => navigation.navigate('SalesInvoiceDetail', { invoiceId: item.name })}
      ...
      >
        <View style={styles.cardTop}>
          <View style={styles.orderMeta}>
            ...
            <Text style={styles.customerName} numberOfLines={1}>{item.customer_name || item.customer}</Text>
          </View>
        ...
        <View style={styles.cardBottom}>
          <View>
            ...
            <Text style={styles.valueText}>{formatDate(item.posting_date)}</Text>
          </View>
  ...
  return (
    <ModuleLayout title="Sales Invoices" user={user} showBack={true}>
      <FilterHeader 
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <View style={styles.emptyIconContainer}>
                <Receipt size={32} color={colors.neutral_300} />
              </View>
              <Text style={styles.emptyTitle}>No Invoices Found</Text>
              <Text style={styles.emptySubtitle}>Adjust your filters or create a new invoice.</Text>
            </View>
          }
          refreshControl={
