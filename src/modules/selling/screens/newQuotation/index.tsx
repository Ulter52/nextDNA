...
import { User, Calendar, FileText, CheckCircle2, Package, Tag, Building2, Plus, Trash2, ShoppingCart, Percent, Clock, Briefcase, LayoutGrid } from 'lucide-react-native';
...
const quotationSchema = z.object({
  ...
  items: z.array(quotationItemSchema).min(1, 'At least one item'),
});
...
const DateSection = ({ watch, setValue }: { watch: any, setValue: any }) => {
  const [showPicker, setShowPicker] = useState<'transaction_date' | 'valid_till' | null>(null);
  ...
  return (
    <>
      ...
      <TouchableOpacity onPress={() => setShowPicker('valid_till')} activeOpacity={0.7}>
        <View pointerEvents="none">
          <FormInput label="Valid Till" value={validTill} editable={false} icon={Clock} />
        </View>
      </TouchableOpacity>
      {showPicker && (
        <DateTimePicker 
...
export function NewQuotation() {
  ...
  const { control, handleSubmit, watch, setValue, reset } = useForm<QuotationFormValues>({
  ...
  useEffect(() => {
    if (isEdit && quotationDetail) reset(quotationDetail);
  }, [isEdit, quotationDetail, reset]);
  ...
  const onSubmit = async (data: QuotationFormValues) => {
    try {
      const res = await saveMutation.mutateAsync({ data, id: quotationId });
      Alert.alert("Success", "Quotation saved");
        ...
        <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          {section.id === 'basic' && (
            <View>
              ...
              <Controller control={control} name="party_name" render={({ field: { onChange, value } }) => (
                <Selector 
              ...
              <Controller control={control} name="project" render={({ field: { onChange, value } }) => (
                <Selector 
                  ...
                  loadingNextPage={isFetchingNextProjects} icon={Briefcase} placeholder="Select Project" 
                />
              )} />
              <Controller control={control} name="cost_center" render={({ field: { onChange, value } }) => (
                <Selector 
                  ...
                  loadingNextPage={isFetchingNextCostCenters} icon={LayoutGrid} placeholder="Select Cost Center" 
                />
              )} />
            </View>
          )}
          ...
          {section.id === 'taxes' && (
            <View>
              ...
              <Controller control={control} name="selling_price_list" render={({ field: { onChange, value } }) => (
                <Selector label="Price List" options={[{ name: 'Standard Selling' }, { name: 'Standard Buying' }]} value={value} onChange={onChange} icon={ShoppingCart} />
              )} />
            </View>
          )}

          {section.id === 'save' && (
            <SaveSection isEdit={isEdit} isPending={saveMutation.isPending} handleSubmit={handleSubmit(onSubmit)} title={isEdit ? "Update Quotation?" : "Save Quotation?"} />
          )}
        </ScrollView>
      </View>
    );
  }, [control, companies, customers, items, projects, costCenters, taxCategories, taxTemplates, loadingTaxes, fields, append, remove, watchCurrency, watchItems, isEdit, saveMutation.isPending, handleSubmit, onSubmit, setValue, watch, hasNextCustomers, fetchNextCustomers, isFetchingNextCustomers, hasNextItems, fetchNextItems, isFetchingNextItems, hasNextProjects, fetchNextProjects, isFetchingNextProjects, hasNextCostCenters, fetchNextCostCenters, isFetchingNextCostCenters]);
